...
def search_person(request):
    ...
    if request.method=="POST":
         ...
         if search_param_len == 1:
             ...
             customers = md.Customer.objects.filter(Q(name__icontains=search_param[0]) | Q(last__icontains=search_param[0]))
             print(customers)
...
def add_person(request):
    if request.method=="POST":
        ...
        order=md.Order.objects.create(customer=customer, date_created=dt.now(),description=description,price=int(price))
